...
class CantinaApp:
    ...
    def __init__(self, root):
        ...
        # Muestra si algun producto está por debajo del stock mínimo al abrir el programa
    ...
    def menu_proveedores(self):
        ...
        self._crear_submenu(
            "MENÚ DE PROVEEDORES",
            [
                ("Ver proveedores", self.ver_proveedores),
    ...
    def ver_productos(self):
        ...
        tree.tag_configure('stock_cero', background='#ffcccc')
        tree.tag_configure('stock_minimo', background='#fff7b2')
    ...
    def registrar_producto(self):
        ...
        # Solo productos agregados manualmente (marcados con manual = True, ej.: mixto, hamburguesa)
        ...
        def autocompletar_campos(*args):
            ...
            if producto:
                ...
                # Limpia los campos excepto nombre
    ...
    def registrar_cliente(self):
        """
        Abre un formulario para registrar un nuevo cliente (alumno o profesor).
        """
        win = tk.Toplevel(self.root)
        ...
        # Actualizar campos según tipo de cliente
    ...
    def ver_clientes(self):
        ...
        # Cargar clientes en la tabla con filtro
        def cargar_clientes():
            tree.delete(*tree.get_children())
            filtro = var_buscar.get().lower()
            for c in self.clientes:
                ...
                prepago = f"Gs/{getattr(c, 'saldo_prepago', 0):.2f}" if hasattr(c, 'saldo_prepago') else "-"
                deuda = f"Gs/{getattr(c, 'deuda', 0):.2f}" if hasattr(c, 'deuda') and getattr(c, 'deuda', 0) > 0 else "-"
        ...
        def mostrar_detalle(event):
            ...
            prepago = f"Gs/{getattr(cliente, 'saldo_prepago', 0):.2f}" if hasattr(cliente, 'saldo_prepago') else "-"
            deuda = f"Gs/{getattr(cliente, 'deuda', 0):.2f}" if hasattr(cliente, 'deuda') and getattr(cliente, 'deuda', 0) > 0 else "-"
    ...
    def recargar_prepago_cliente(self):
        ...
        # Llenar la lista con los alumnos
    ...
    def pagar_deuda_cliente(self):
        ...
        # Llenar la lista con los clientes con deuda
        def actualizar_lista(*args):
            filtro = var_buscar.get().lower()
            listbox.delete(0, tk.END)
        ...
        # Mostrar deuda al seleccionar un cliente
        def mostrar_deuda(event=None):
            ...
            if cliente:
                label_deuda.config(text=f"Deuda actual: Gs/{getattr(cliente, 'deuda', 0):.2f}")
        ...
        def guardar():
            ...
            if monto > cliente.deuda:
                messagebox.showerror("Error", f"El monto no puede ser mayor a la deuda actual (Gs/{cliente.deuda:.2f}).", parent=win)
                return
            metodo = var_metodo.get()
            if metodo == "Caja":
                self.caja.ingresar_dinero(monto)
    ...
    def agregar_proveedor(self):
        """
        Muestra un formulario para agregar un proveedor con validación.
        """
        win = tk.Toplevel(self.root)
    ...
    def ver_proveedores(self):
        """
        Muestra la lista de proveedores con detalles (nombre, contacto, teléfono).
        """
        win = tk.Toplevel(self.root)
        ...
        # Mostrar proveedores en la tabla con filtro
        ...
        # Actualizar la tabla al cambiar el filtro
    ...
    def ver_ventas(self):
        ...
        def mostrar_detalle(event):
            ...
            detalle += f"\nTOTAL: Gs/{total:.2f}" if total is not None else ""
            label_det.config(text=detalle)
        tree.bind('<<TreeviewSelect>>', mostrar_detalle)
    ...
    def ver_compras(self):
        """
        Muestra el historial de compras en una ventana.
        """
        ...
        def mostrar_detalle(event):
            ...
            for nombre, datos in productos.items():
                if len(datos) == 3:
                    ...
                    cant, precio = datos
                detalle += f"- {nombre}: {cant} x Gs/{precio:.2f} = Gs/{cant*precio:.2f}\n"
                total += cant * precio
            detalle += f"\nTOTAL: Gs/{total:.2f}"
            label_det.config(text=detalle)
        tree.bind('<<TreeviewSelect>>', mostrar_detalle)
    ...
    def registrar_venta(self):
        ...
        def actualizar_lista_productos():
            ...
            for nombre, var in cantidad_vars.items():
                ...
                ttk.Label(f, text=f"Precio: Gs/{p.precio:.2f}", width=15).pack(side="left")
                ttk.Label(f, text="Cantidad:").pack(side="left")
                spin = ttk.Spinbox(f, from_=0, to=p.stock, width=5, textvariable=var)
                spin.pack(side="left", padx=5)
                # Subtotal por producto
                label_subtotal = ttk.Label(f, text=f"Subtotal: Gs/{p.precio * var.get():.2f}", width=18)
                ...
                def on_cantidad_change(*args, nombre=nombre, var=var, label=label_subtotal):
                    ...
                    cantidad = var.get()
                    label.config(text=f"Subtotal: Gs/{prod.precio * cantidad:.2f}")
                    actualizar_total()
        ...
        label_total = ttk.Label(win, text="Total: Gs/0.00", font=("Arial", 13, "bold"))
        ...
        def actualizar_total(*args):
            ...
            for nombre, var in cantidad_vars.items():
                ...
                if cant > 0:
                    total += p.precio * cant
            label_total.config(text=f"Total: Gs/{total:.2f}")
        ...
        frame_cliente = ttk.LabelFrame(win, text="Seleccionar Cliente", padding=10)
        ...
        ttk.Button(frame_cliente, text="Cliente", command=seleccionar_cliente).pack(side="left")
        ...
        def guardar():
            ...
            # Validar al menos un producto seleccionado
    ...
    def registrar_compra(self):
        ...
        # Boton para seleccionar proveedor
        ...
        ttk.Label(frame_nuevo, text="(Nombre Producto, Cantidad, Precio Compra, Precio Venta, Categoria, Stock Minimo)").pack(side="left", padx=5)

        # Ocultar campos de venta si no es disponible
        def actualizar_campos_nuevo_producto(*args):
            # Producto disponible a la venta (ej.: gaseosa)
            if var_nuevo_disponible.get():
                entry_nuevo_precio_venta.config(state="normal")
                entry_nuevo_stock_min.config(state="normal")
            # Producto no disponible a la venta (ej.: aceite)
            else:
        ...
        def guardar():
            try:
                ...
                for p, var_cant, var_precio, var_disponible in productos_vars:
                    ...
                    if cant > 0:
                        if disponible:
                            # Solo si es vendible (disponible), agrega al inventario
                ...
                # Validar al menos un producto comprado
    ...
    def ver_caja(self):
        ...
        tk.Label(win, text=f"Gs/ {monto:.2f}", font=("Arial", 28, "bold"), fg="#229954", bg="#f5f5f5").pack(pady=(0, 18))
    ...
    def modificar_caja(self):
        """
        Permite modificar el dinero en caja.
        """
        win = tk.Toplevel(self.root)
        ...
        tk.Label(win, text=f"Dinero actual: Gs/{self.caja.dinero:.2f}", font=("Arial", 12), bg="#f5f5f5").pack(pady=(0, 8))
    ...
    def ver_banco(self):
        ...
        tk.Label(win, text=f"Gs/ {saldo:.2f}", font=("Arial", 28, "bold"), fg="#2980b9", bg="#f5f5f5").pack(pady=(0, 18))
        tk.Button(win, text="Cerrar", command=win.destroy, bg="#e74c3c", fg="white", font=("Arial", 12, "bold"), relief="flat", width=12).pack()

    def modificar_banco(self):
        """
        Permite modificar el saldo en banco.
        """
        win = tk.Toplevel(self.root)
        ...
        tk.Label(win, text=f"Dinero actual: Gs/{self.banco.saldo:.2f}", font=("Arial", 12), bg="#f5f5f5").pack(pady=(0, 8))
        tk.Label(win, text="Nuevo monto:", font=("Arial", 11), bg="#f5f5f5").pack()
        entry_monto = tk.Entry(win, font=("Arial", 12), justify="center")
        entry_monto.pack(pady=(0, 10))
        entry_monto.focus_set()
    ...
    def salir(self):
        ...
        """
        self.alerta_bajo_stock()  # Muestra la alerta antes de cerrar el programa
        Persistencia.guardar(self.inventario, "inventario.pkl")
        Persistencia.guardar(self.caja, "caja.pkl")
        Persistencia.guardar(self.banco, "banco.pkl")
        Persistencia.guardar(self.clientes, "clientes.pkl")
    ...
    # Métodos genéricos y auxiliares
    ...
    def editar_proveedor_individual(self, proveedor):
        """
        Abre un formulario para editar un proveedor.
        """
        win = tk.Toplevel(self.root)
    ...
    def editar_cliente_individual(self, cliente):
        ...
        """
        Abre un formulario para editar un cliente (alumno o profesor).
        """
        win = tk.Toplevel(self.root)
